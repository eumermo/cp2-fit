...
```sql
CREATE USER cp2fit WITH ENCRYPTED PASSWORD 'cp2fit';
GRANT ALL ON SCHEMA public TO cp2fit;
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO cp2fit;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO cp2fit;
```
...
```bash
 psql -h localhost  -d cp2fit -U cp2fit -W -f db/create.sql
```
